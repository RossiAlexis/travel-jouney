...
export async function getTripStats(tripId, userId) {
    ...
    if (trip.startDate && trip.endDate) {
        totalDays = Math.ceil((trip.endDate.getTime() - trip.startDate.getTime()) / (1000 * 60 * 60 * 24));
    }
    ...
    const memoriesByCategory = trip.memories.reduce((acc, m) => {
        const cat = m.category ?? "general";
        acc[cat] = (acc[cat] ?? 0) + 1;
        return acc;
    }, {});
    const locationSet = new Set(trip.memories
...
export async function exportTripAsJson(tripId, userId) {
    const trip = await db.trip.findFirst({
        where: { id: tripId, userId },
        include: {
            memories: {
